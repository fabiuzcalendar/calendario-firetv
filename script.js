...
(function kioskDiagnostics(){
  ...
  let box = document.getElementById("kiosk-diag");
  if (!box) {
  ...
  function snap(){
    ...
    const scaleVar = cs.getPropertyValue("--kiosk-scale").trim();

    box.textContent =
      `viewport ${w}x${h} dpr=${dpr} | ` +
      `appRect ${Math.round(appRect.width)}x${Math.round(appRect.height)} | ` +
      `scaleVar=${scaleVar || "(none)"} | ` +
      `transform=${tr}` +
      (calRect ? ` | calendarRect ${Math.round(calRect.width)}x${Math.round(calRect.height)}` : "");
  }
